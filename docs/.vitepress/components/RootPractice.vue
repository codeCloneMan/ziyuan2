...
const handleInput = (e) => {
  ...
  if (input.length > 1) {
    ...
    userInput.value = input.slice(0, 1)
  }
  
  if (input.length === 1 && !isComplete.value) {
    answeredRoots.value++
...
const handleBeforeUnload = () => {
  if (fontLoaded.value && !isComplete.value && answeredRoots.value > 0) {
    saveProgress(
      practiceMode.value,
      correctCount.value,
      answeredRoots.value,
